...
public class FileConfigServiceImpl implements FileConfigService {
    ...
    private final LoadingCache<Long, FileClient> clientCache = CacheUtils.buildAsyncReloadingCache(Duration.ofSeconds(10L),
            new CacheLoader<>() {
                ...
                @NotNull
                @Override
                public FileClient load(@NotNull Long id) {
    ...
    public void deleteFileConfig(Long id) {
        ...
        if (Boolean.TRUE.equals(config.getMaster())) {
            throw ServiceExceptionUtil.get(FILE_CONFIG_DELETE_FAIL_MASTER);
        }
        // 删除
    ...
    private FileConfigDO validateFileConfigExists(Long id) {
        ...
        if (config == null) {
            throw ServiceExceptionUtil.get(FILE_CONFIG_NOT_EXISTS);
        }
        return config;
    }
