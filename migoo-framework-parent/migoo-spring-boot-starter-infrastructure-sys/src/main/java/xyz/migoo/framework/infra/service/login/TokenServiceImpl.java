...
public class TokenServiceImpl implements TokenService {
    ...
    public UserDetails loadUserByUsername(String jsonUser) {
        ...
        BaseUser user = this.getUser(req.getUsername());
    ...
    public AuthLoginRespVO signIn(AuthLoginReqVO req) {
        OnesLoginUser loginUser = login0(JsonUtils.toJsonString(req), req.getPassword());
        // 缓存登陆用户到 Redis 中，返回 sessionId 编号
        String token = securitySessionAuthService.createUserSession(loginUser);
        return new AuthLoginRespVO().setToken(token)
                .setRequiredBindAuthenticator(loginUser.getRequiredBindAuthenticator());
    }

    private BaseUser getUser(String username) {
        // 根据请求传递的用户类型 查询对应的表数据
        return userService.get(username);
    }

    private OnesLoginUser login0(String reqJson, String password) {
        try {
            ...
            return (OnesLoginUser) authentication.getPrincipal();
    ...
    private void refreshLoginUserCache(String token, LoginUser loginUser) {
        ...
        if (System.currentTimeMillis() - loginUser.getUpdateTime().getTime() > securitySessionAuthService.getSessionTimeoutMillis() / N_2) {
            ...
            BaseUser user = getUser(loginUser.getUsername());
