...
public class RSA {
    ...
    public static String decrypt(String content, PrivateKey privateKey, String algorithm) {
        try {
            ...
            byte[] signed = cipher.doFinal(Base64.getDecoder().decode(content));
    ...
    public static String toBase64String(String privateKey, String pwd) {
        try {
            ...
            char[] password = (pwd == null || pwd.isEmpty()) ? null : pwd.toCharArray();
            keyStore.load(new FileInputStream(privateKey), password);
            return Base64.getEncoder().encodeToString(keyStore.getKey(keyStore.aliases().nextElement(), password).getEncoded());
        } catch (Exception e) {
            throw ServiceExceptionUtil.get(-1, "密钥转换失败，文件不存在或文件已损坏");
        }
    }
